...
f = open('steps1-non.csv', mode='a', encoding='utf-8', newline='')
csv_writer = csv.DictWriter(f, fieldnames=['Part_Number','SKU', 'Make','link'])
csv_writer.writeheader()
df = pd.read_excel('part.xlsx')
sku_numbers = df['sku']
part_numbers = df['part']
for sku_number,part_number in zip(sku_numbers,part_numbers):
    params = {
                    "country": "USA",
                    "customerType": "B2C",
                    "salesChannel": "ECOMM",
    ...
    # 基础URL
    base_url = f"https://www.autozone.com/external/product-discovery/product/v1/products/{sku_number}/fitments"
    ...
    print(full_url)
    make = requests.get(url=full_url,headers=headers).text
    js_data = json.loads(make)
